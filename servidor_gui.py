...
class ServidorApp(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("Servidor Holograma (Áudio+Vídeo)")
    ...
    def _build_interface(self) -> None:
        ...
        ctk.CTkComboBox(holo_frame, values=["raw","mirror","quad2x2","cross"], variable=self.mode_var, width=120).grid(row=0, column=1, sticky="w")
        ...
        ctk.CTkComboBox(
            holo_frame,
            values=["Nenhum", "Cinza", "Sépia", "Blue BGR", "Desfoque", "Canny"],
    ...
    def _audio_capture_loop(self) -> None:
        """
        Captura áudio contínuo e coloca chunks na fila.
        """
        try:
        ...
        try:
            while self.running.is_set():
                try:
                    ...
                    # silêncio: 2 bytes por amostra * canais * chunk
                    data = b"\x00" * (self.chunk * 2 * self.channels)
                try:
                    self.audio_queue.put_nowait(data)
                except queue.Full:
                    # descarta o mais antigo e insere o novo
                    try:
    ...
    def capture_loop(self) -> None:
        ...
        try:
            while self.running.is_set():
                ...
                if ok:
                    # Coleta todos os chunks de áudio acumulados desde o último frame
                    audio_chunks = []
                    # Limita quantidade máxima para evitar payload gigantesco (opcional)
                    max_chunks_take = 50
                    taken = 0
